...
//sunny opt
//1. pool element
//2. destroy mMeshObject when destroyed.
...
namespace Xft
{
    public class XWeaponTrail : MonoBehaviour
    {
        ...
        private static Pool<Element> elementPool = new Pool<Element>();
        ...
        public class Element
        {
            ...
            //    PointEnd = end;
            //}
            public Element reset(Vector3 start, Vector3 end) {
        ...
        public void OnDestroy() {
            if (mMeshObj != null) GameObject.DestroyImmediate(mMeshObj);
        }
        ...
        void RecordCurElem()
        {
            ...
            Element elem = elementPool.get().reset(PointStart.position, PointEnd.position);
            ...
            {
                int index = mSnapshotList.Count - 1;
                elementPool.free(mSnapshotList[index]);
                mSnapshotList.RemoveAt(index);
                mSnapshotList.Insert(1, elem);
            }
        ...
        void InitOriginalElements()
        {
            ...
            mSnapshotList.Add(elementPool.get().reset(PointStart.position, PointEnd.position));
            mSnapshotList.Add(elementPool.get().reset(PointStart.position, PointEnd.position));
        }
